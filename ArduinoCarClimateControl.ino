...
void handleDataGetRequest() {
  server.send(200, "text/plane", String(measured_temperature_1) + "|" + String(estimated_temperature_1) + "|" + String(temperature1_derivative1) + "|" + String(temperature1_derivative2) + "|" + String(measured_temperature_2) + "|" + String(measured_temperature_3) + "|" + String(salon_temperature_wanted) + "|" + String(stream_temperature_wanted) + "|" + String(pot_real_percent) + "|" + String(servo_pos_percent) + "|" + String(light_reading_smooth));
}
    ...
    if (pot_real_percent < 7) {
      ...
      if (measured_temperature_1 > 200 || k1 == 0) {  //мануальный режим при первом коэффициенте ==0 либо нерабочем датчике
        ...
        if (abs(pot_real_percent - pot_smooth_percent) > POT_RAPID_MOVEMENT_GATE) {
          if (abs(pot_real_percent - pot_smooth_percent) > POT_RAPID_MOVEMENT_GATE2){
              servo_pos_percent = pot_real_percent;
            }
          else 
            if (sign(pot_real_percent - pot_smooth_percent) == sign(pot_real_percent - servo_pos_percent)) {
              servo_pos_percent = pot_real_percent;  //резко двигаем серво на новую позицию только если это будет движение на отдаление от текущего положения серво
            }
